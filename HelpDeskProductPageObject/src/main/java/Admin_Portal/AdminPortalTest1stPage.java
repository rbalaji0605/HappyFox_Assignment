...
Comment 1 : For POM design pattern, class AdminPortalTest1stPage should be under folder structure 

src/main/java/PageObjectModel/Admin_Portal/FirstPage

Comment 2 : Class name should be much more readable or page title can be used for class name
...
public class AdminPortalTest1stPage extends AppPage {
	...
	Comment 3 : @CacheLookup can be used to improve the performance of locating the already visited element in the cache
	
	Comment 4 : LinkText locator can be avoided when there is a possibility of same text present and we can use unique identifier like id,cssSelector or xpath

	Comment 5: [Optional] Ordering of elements and their methods can be changed like all the definition of elements should be done, followed by their methods or actions. To improve readability

	Comment 6 : ClickTitle() can be created as a separate method and reduce the extra lines of code and code efficiency
	...
	*/
	...
	Comment 7: Multiple sleep statement is used. Instead we should use explicit wait or fluent wait to handle the wait dynamically and improve the execution 
